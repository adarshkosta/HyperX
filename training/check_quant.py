...
def act_dquant(inp, B):
    I = 0
    # act = torch.clamp(inp, 1/n, 1-1/n)

    act = torch.clamp(inp, -2**I, 2**I-1/2**B)

    return torch.floor(act*2**B)/2**B
...
def wt_dquant(inp, B):
    ...
    wt = torch.clamp(wt, -2**I, 2**I-1/2**B)

    return torch.floor(wt*2**B)/2**B


def act_fsquant(inp, B):
    S = 1
    I = 0
    F = B-I-S

    return torch.floor((torch.clamp(inp, -2**I, 2**I-1/2**F)/2**I)*2**(B-1))/2**(B-1)

def wt_fsquant(inp, B):
    S = 1
    I = 0
    F = B-I-S
...
print('D-Quant:',torch.unique(act_dquant(inp=A, B=7)))
print(len(torch.unique(act_dquant(inp=A, B=7))))

print('FS-Quant:', torch.unique(act_fsquant(inp=A, B=B)))
print(len(torch.unique(act_fsquant(inp=A, B=B))))


print('D-Quant:',torch.unique(wt_dquant(inp=W, B=7)))
print(len(torch.unique(wt_dquant(inp=W, B=7))))

print('FS-Quant:',torch.unique(wt_fsquant(inp=W, B=B)))
print(len(torch.unique(wt_fsquant(inp=W, B=B))))
