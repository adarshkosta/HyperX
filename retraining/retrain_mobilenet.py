...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 11 21:16:27 2020
...
#        print('Index = ', index)
        if args.frozen_layers == 0:
            x = torch.load(os.path.join(self.datapath, 'act_ConvBNReLU1'+'_'+str(index)+'.pth.tar'))
        elif args.frozen_layers == 18:
            x = torch.load(os.path.join(self.datapath, 'act_ConvBNReLU2'+'_'+str(index)+'.pth.tar'))
        elif args.frozen_layers == 19:
            x = torch.load(os.path.join(self.datapath, 'act_fc'+'_'+str(index)+'.pth.tar'))
        else: 
            x = torch.load(os.path.join(self.datapath, 'act_InvertedResidual'+str(self.args.frozen_layers)+'_'+str(index)+'.pth.tar'))
...
elif args.frozen_layers == 0:
    model = (__import__(args.model+'_freeze_convbn1'))
elif args.frozen_layers == 18:
    model = (__import__(args.model+'_freeze_convbn2'))
else:
    ...
    if args.pretrained: #Initialize params with pretrained model
        ...
        for name1, m1 in model.named_modules():
            for name2, m2 in original_model.named_modules():
                if name2 == name1:
                    if 'InvertedResidual' in name1 or 'ConvBNReLU' in name1 or 'fc' in name1:
                        m1.load_state_dict(m2.state_dict())
        
        #Re-build classifier layer
        if args.dataset == 'cifar10':
            model.fc = nn.Linear(1280, 10, bias=False)
        elif args.dataset == 'cifar100':
            model.fc = nn.Linear(1280, 100, bias=False)
        else:
            raise Exception(args.dataset + 'is currently not supported')
...
if args.frozen_layers == 19:
    ...
    datapath_train = os.path.join(args.load_dir, args.dataset, args.model, 'train', 'ConvBNReLU2')
    datapath_test = os.path.join(args.load_dir, args.dataset, args.model, 'test', 'ConvBNReLU2')
elif args.frozen_layers == 0:
    datapath_train = os.path.join(args.load_dir, args.dataset, args.model, 'train', 'ConvBNReLU1')
    datapath_test = os.path.join(args.load_dir, args.dataset, args.model, 'test', 'ConvBNReLU1')
else:
    datapath_train = os.path.join(args.load_dir, args.dataset, args.model, 'train', 'InvertedResidual' + str(args.frozen_layers))
    datapath_test = os.path.join(args.load_dir, args.dataset, args.model, 'test', 'InvertedResidual' + str(args.frozen_layers))
