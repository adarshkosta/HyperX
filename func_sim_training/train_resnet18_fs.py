...
parser.add_argument('-b', '--train-batch-size', default=320, type=int,
                    metavar='N', help='mini-batch size (default: 128)')
parser.add_argument('-s', '--test-batch-size', default=320, type=int,
                    metavar='N', help='mini-batch size (default: 128)')
            ...
            if args.half:
                if args.tag:
                    save_checkpoint({
                        ...
                        'best_acc': best_acc,
                    }, is_best, path= args.savedir, filename=args.model +'fxp_' + args.dataset + '_half_' + str(args.tag))
                else:
                    save_checkpoint({
                        'state_dict': model.state_dict(),
                        'best_acc': best_acc,
                    }, is_best, path= args.savedir, filename=args.model +'fxp_' + args.dataset + '_half')
            else:
                if args.tag:
                    save_checkpoint({
                        'state_dict': model.state_dict(),
                        'best_acc': best_acc,
                    }, is_best, path=args.savedir, filename=args.model +'fxp_' + args.dataset + '_full_' + str(args.tag))
                else:
                    save_checkpoint({
                        'state_dict': model.state_dict(),
                        'best_acc': best_acc,
                    }, is_best, path= args.savedir, filename=args.model +'fxp_' + args.dataset + '_full')
