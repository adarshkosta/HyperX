...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 19:44:01 2020
...
rs_models_dir = os.path.join(root_dir, "rs_models")
datasets_dir = os.path.join(root_dir, "datasets")

sys.path.insert(0, root_dir) # 1 adds path to end of PYTHONPATH
sys.path.insert(0, models_dir)
sys.path.insert(0, rs_models_dir)
...
model_names = []
for path, dirs, files in os.walk(rs_models_dir):
    for file_n in files:
...
#Parse arguments
parser = argparse.ArgumentParser(description= 'Test-RS')
...
parser.add_argument('--rram-layers', default=3, type=int, help='number of layers from left on rram in the model')
...
#Create model
print('==> Building RS-model for', args.model, '...')
if (args.model+'_rram'+str(args.rram_layers) in model_names):
    model = (__import__(args.model+'_rram'+str(args.rram_layers))) 
else:
