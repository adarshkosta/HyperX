...
def compare_acts(layer, metric='mean', idx=0):
    ...
    if metric == 'mean':
        ...
        err_mean = torch.cat((err_mean, torch.mean(acts1-acts2)), dim=0) #mean1 - mean2
...
metrics = np.stack((mse, mean_diff), axis=-1)
print(metrics)

np.savetxt(os.path.join(save_path, 'metrics_cf100.csv'), metrics, delimiter=',')
